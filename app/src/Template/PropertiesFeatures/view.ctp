<?php
/**
 * @var \App\View\AppView $this
 * @var \App\Model\Entity\PropertiesFeature $propertiesFeature
 */
?>
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        <li class="heading"><?= __('Actions') ?></li>
        <li><?= $this->Html->link(__('Edit Properties Feature'), ['action' => 'edit', $propertiesFeature->id]) ?> </li>
        <li><?= $this->Form->postLink(__('Delete Properties Feature'), ['action' => 'delete', $propertiesFeature->id], ['confirm' => __('Are you sure you want to delete # {0}?', $propertiesFeature->id)]) ?> </li>
        <li><?= $this->Html->link(__('List Properties Features'), ['action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Properties Feature'), ['action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Properties'), ['controller' => 'Properties', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Property'), ['controller' => 'Properties', 'action' => 'add']) ?> </li>
        <li><?= $this->Html->link(__('List Features'), ['controller' => 'Features', 'action' => 'index']) ?> </li>
        <li><?= $this->Html->link(__('New Feature'), ['controller' => 'Features', 'action' => 'add']) ?> </li>
    </ul>
</nav>
<div class="propertiesFeatures view large-9 medium-8 columns content">
    <h3><?= h($propertiesFeature->id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Property') ?></th>
            <td><?= $propertiesFeature->has('property') ? $this->Html->link($propertiesFeature->property->id, ['controller' => 'Properties', 'action' => 'view', $propertiesFeature->property->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Feature') ?></th>
            <td><?= $propertiesFeature->has('feature') ? $this->Html->link($propertiesFeature->feature->name, ['controller' => 'Features', 'action' => 'view', $propertiesFeature->feature->id]) : '' ?></td>
        </tr>
        <tr>
            <th scope="row"><?= __('Id') ?></th>
            <td><?= $this->Number->format($propertiesFeature->id) ?></td>
        </tr>
    </table>
</div>
